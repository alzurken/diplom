...
public class ImportDataInput extends ImportData
{
    ...
    protected void process(String str)
    {
        ...
        sdata += "-" + str.substring(0, str.indexOf(','));
        ...
        // High
        buf = str.substring(0, str.indexOf(','));
        high = Double.parseDouble(buf);
        str = str.substring(str.indexOf(',') + 1);
        // Volume
        volume = Double.parseDouble(str);
        data.add(new Bar(date, open, close, low, high));
        // "add" method adds item to the end of the list, so we need sort data in file from newest
        // to latest
    }
